...
def import_espn_league(league_id, year, week):
    """ Import league data from espn and store in local data structure. 
        ...
        Args:
            league_id: unique espn id parsed from league url.
            year: year to import.
            week: current week of the nfl/fantast football season.
    """
...
def build_standings(teams):
    """ Initialize a standings dictionary. 
    
        Args:
            teams: List of teams in the league.
        Returns: A dictionary containing each team and their current win count.
    """
...
def remove_irrelevant_outcomes(playoff_scenarios, team, league):
    """ Removes all individual outcomes that do not directly effect whether team 'team' makes the playoffs.

        Args:
            playoff_scenarios: List of every possible playoff scenario for team 'team'.
            team: Team instance.
            league: League instance.
    """
...
def wins_needed(standings, num_playoff_teams, playoff_team):
    """ Calculates the minimum number of wins a team needs to make the playoffs assuming they will win out. 
        ...
        Args: 
            standings: Dictionary of the current standings, key = team, value = win count.
            num_playoff_teams: Number of teams who will make the playoffs.
            playoff_team: Team being calculated. Team instance.
    """
...
def build_readable_json_structure(json, scenario_list):
    """ Convert list of scenarios into readable json structure. 
        ...
        Args:
            ...
            scenario_list: Full list of playoff scenarios.
    """
...
def compile_scenario_list(scenario_structure, path):
    """ Transforms scenario structure into an iterable list.

        Args:
            scenario_structure: ScenarioStructure instance.
            path: list of matchups in current recursive set.
    """
    scenarios = []
    if not scenario_structure.next:
        scenario_data = {}
        outcome = {}
        for team, value in scenario_structure.value.items():
        ...
        scenario_data['tiebreaker'] = scenario_structure.tiebreaker
        scenario_data['scenarios'] = scenarios[0]
        return [scenario_data]
    else:
        next_path = path.copy()
        next_path.append(scenario_structure.value)
        for next in scenario_structure.next:
